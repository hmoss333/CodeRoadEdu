...
public class Abilities2 : MonoBehaviour
{
    ...
    bool loopAnswerCorrect()
    {
        ...
        try
        {
            if (showMoves.text.Equals("Begin Loop...Spin..." + startFormat + "End Loop" + endFormat + "...") && loopsFromSlider == 3)
                return true;
            ...
            for (int i = movement.Count - 1; i >= 0; i--)
            {
                ...
                {
                    Func<bool> forwardLoop = () => movement[i + 1].Equals("Spin") && movement[i + 2].Equals("End Loop");
                    Func<bool> endForwardLoop = () => movement[i + 1].Equals("Spin") && movement[i + 2].Equals(startFormat + "End Loop" + endFormat);
            ...
            if (movement.Count == 6 && countedLoops == 2 && loopPatternCorrect())
                return true;
            if (movement[0].Equals("Spin") && countedLoops == 1 && loopCounts[0] == 2)
                return true;
            if (movement[3].Equals(startFormat + "Spin" + endFormat) && countedLoops == 1 && loopCounts[0] == 2)
                return true;
    ...
    {
        //SceneManager.LoadSceneAsync("Abilities3");
        narration.Stop();
        MiniGame.UnloadScene(MiniGame.Level.Level5);
        MiniGame.LoadScene(MiniGame.Level.Level6);
    }
