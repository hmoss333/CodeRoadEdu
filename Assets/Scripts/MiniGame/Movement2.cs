...
public class Movement2 : MonoBehaviour
{
    ...
    bool loopAnswerCorrect()
    {
        ...
        try
        {
            if (showMoves.text.Equals("Begin Loop...Backward..." + startFormat + "End Loop" + endFormat + "...") && loopsFromSlider == 4)
                return true;
            ...
            for (int i = movement.Count - 1; i >= 0; i--)
            {
                ...
                {
                    Func<bool> forwardLoop = () => movement[i + 1].Equals("Backward") && movement[i + 2].Equals("End Loop");
                    Func<bool> endForwardLoop = () => movement[i + 1].Equals("Backward") && movement[i + 2].Equals(startFormat + "End Loop" + endFormat);
            ...
            if (movement.Count == 6 && countedLoops == 2 && loopPatternCorrect())
                return true;
            if (movement[0].Equals("Backward") && countedLoops == 1 && loopCounts[0] == 3)
                return true;
            if (movement[3].Equals(startFormat + "Backward" + endFormat) && countedLoops == 1 && loopCounts[0] == 3)
                return true;
            if (movement[0].Equals("Backward") && movement[1].Equals("Backward") && countedLoops == 1 && loopCounts[0] == 2)
                return true;
            if (movement[3].Equals("Backward") && movement[4].Equals(startFormat + "Backward" + endFormat) && countedLoops == 1 && loopCounts[0] == 2)
                return true;
            if (movement[0].Equals("Backward") && movement[4].Equals(startFormat + "Backward" + endFormat) && countedLoops == 1 && loopCounts[0] == 2)
                return true;
    ...
    void checkCorrect()
    {
        if (showMoves.text.Equals("Backward...Backward...Backward..." + startFormat + "Backward" + endFormat + "..."))
        {
            displayWinScreen();
        }
